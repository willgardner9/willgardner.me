var set_emoji_favicon = (function () {
  ...
  if (is_worker) {
    ...
    context_1.font =
      "normal normal normal " + size_1 + "px/" + size_1 + "px sans-serif";
    ...
    var lastlink = [].slice
      .call(document.getElementsByTagNameNS(ns, "link"), 0)
      .filter(function (link) {
        return (
          link.rel.toLowerCase() === "icon" && mime_text_regex_1.test(link.type)
        );
      })
      .pop();
    if (lastlink) {
      var xhr_1 = new XMLHttpRequest();
      var uri = lastlink.href
        .trim()
        .replace(/^data:(;base64)?,/, "data:text/plain;charset=utf-8$1,");
      xhr_1.open("GET", uri);
